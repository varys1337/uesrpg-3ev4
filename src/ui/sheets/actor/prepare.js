/**
 * Data preparation helpers for PC actor sheets.
 * 
 ...
 * Re-exported from combat-actions-utils.js for consolidation.
 * 
...
export function buildCombatActionsContext(actor) {
  try {
    ...
    return {
      ...
      canCastMagic: Boolean(actor?.items?.some?.(i => i.type === "spell")),
      canCastInstantMagic: Boolean(actor?.items?.some?.(i => i.type === "spell" && i?.system?.isInstant === true))
    };
  } catch (_e) {
    return { 
      activeCombatStyleId: "", 

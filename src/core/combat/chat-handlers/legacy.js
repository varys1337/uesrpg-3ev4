...
async function _onMagicOpposedAction(ev, message) {
  ev.preventDefault();
  const btn = ev.currentTarget;
...
export function initializeChatHandlers() {
  ...
  Hooks.on("renderChatMessageHTML", (message, html) => {
    ...
    // NOTE: Click handlers are delegated at the ChatLog level (Stage 05b) to avoid per-message listener binding.
    // This hook retains only permission-aware button state and render-time DOM normalization.
    ...
    // Special Action opposed test card buttons (permission gating only; click handled by delegation).

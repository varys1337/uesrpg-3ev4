/**
 * Generated from template.json on 2026-04-10 for TypeDataModel registration.
 * Do not hand-edit unless the generator input changes.
 */

export const ACTOR_TYPE_MODEL_SEEDS = {
  "Player Character": {
    "templates": [
      "equippedWeapons",
      "favorites",
      "combatTracking"
    ],
    "race": "",
    "birthsign": "",
    "age": "",
    "height": "",
    "weight": "",
    "elite_adv": "",
    "wounded": false,
    "woundPenalty": 0,
    "woundNotes": "",
    "bio": "",
    "journal": "",
    "notes": "",
    "paperDoll": "",
    "resistance": {
      "physicalR": 0,
      "fireR": 0,
      "frostR": 0,
      "shockR": 0,
      "magicR": 0,
      "poisonR": 0,
      "diseaseR": 0,
      "natToughness": 0,
      "sunlightR": 0,
      "silverR": 0
    },
    "resistances": {
      "disease": 0,
      "poison": 0,
      "paralysis": 0,
      "bleed": 0,
      "fear": 0,
      "magic": 0
    },
    "traits": {
      "resistance": {
        "fire": 0,
        "frost": 0,
        "shock": 0,
        "acid": 0,
        "poison": 0,
        "disease": 0,
        "bleed": 0
      }
    },
    "recovery": {
      "naturalHealing": {
        "multiplier": 1,
        "flatBonus": 0
      },
      "magickaRecovery": {
        "multiplier": 1
      },
      "staminaRecovery": {
        "multiplier": 1
      }
    },
    "resources": {
      "magicka": {
        "maxBonus": 0
      },
      "stamina": {
        "maxBonus": 0
      },
      "health": {
        "maxBonus": 0
      }
    },
    "movement": {
      "speedBonus": 0,
      "fallDamage": {
        "multiplier": 1
      },
      "ignoreDifficultTerrain": false
    },
    "senses": {
      "lowLightVision": false,
      "darkvision": false,
      "nightSightBonus": 0
    },
    "lucky_numbers": {
      "ln1": 0,
      "ln2": 0,
      "ln3": 0,
      "ln4": 0,
      "ln5": 0,
      "ln6": 0
    },
    "unlucky_numbers": {
      "ul1": 0,
      "ul2": 0,
      "ul3": 0,
      "ul4": 0,
      "ul5": 0
    },
    "supply": "",
    "wealth": 0,
    "current_enc": 0,
    "carry_rating": {
      "current": 0,
      "max": 0,
      "bonus": 0,
      "penalty": 0,
      "label": "Minimal"
    },
    "characteristics": {
      "str": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "STR"
      },
      "end": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "END"
      },
      "agi": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "AGI"
      },
      "int": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "INT"
      },
      "wp": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "WP"
      },
      "prc": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "PRC"
      },
      "prs": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "PRS"
      },
      "lck": {
        "value": 0,
        "total": 0,
        "base": 0,
        "bonus": 0,
        "favored": false,
        "name": "LCK"
      }
    },
    "hp": {
      "base": 0,
      "value": 0,
      "max": 0,
      "temp": 0,
      "bonus": 0
    },
    "tempHP": 0,
    "buffers": {
      "physical": 0,
      "magical": 0,
      "elemental": 0
    },
    "action_points": {
      "value": 0,
      "max": 3
    },
    "luck_points": {
      "value": 0,
      "max": 0,
      "bonus": 0
    },
    "stamina": {
      "value": 0,
      "max": 0,
      "temp": 0,
      "bonus": 0
    },
    "magicka": {
      "value": 0,
      "max": 0,
      "bonus": 0
    },
    "worship": {
      "primaryDomainKey": "",
      "domains": {}
    },
    "xp": 0,
    "xpTotal": 0,
    "campaignRank": "",
    "linguistics": {
      "max": 0,
      "known": ""
    },
    "social": {
      "languages": {
        "entries": []
      },
      "factions": []
    },
    "wound_threshold": {
      "value": 0,
      "base": 0,
      "bonus": 0,
      "traitBonus": 0,
      "talentBonus": 0
    },
    "initiative": {
      "value": 0,
      "base": 0,
      "bonus": 0,
      "traitBonus": 0,
      "talentBonus": 0
    },
    "speed": {
      "value": 0,
      "base": 0,
      "bonus": 0,
      "traitBonus": 0,
      "talentBonus": 0,
      "swimSpeed": 0,
      "flySpeed": 0
    },
    "fatigue": {
      "level": 0,
      "penalty": 0,
      "bonus": 0
    },
    "size": "",
    "armor_class": "",
    "modifiers": {
      "combat": {
        "attackTN": 0,
        "evadeAoOCost": 0,
        "attackLimit": {
          "total": 0,
          "melee": 0,
          "ranged": 0
        },
        "opposed": {
          "attackTN": 0
        },
        "defenseTN": {
          "total": 0,
          "evade": 0,
          "block": 0,
          "parry": 0,
          "counter": 0
        }
      },
      "movement": {
        "climbSpeed": 0,
        "dashMultiplier": 0,
        "sprintMultiplier": 0,
        "hiddenSpeedMultiplier": 0
      },
      "magic": {
        "negateChance": 0,
        "spellRestraintBonus": 0,
        "cost": {
          "_all": 0,
          "alteration": 0,
          "conjuration": 0,
          "destruction": 0,
          "illusion": 0,
          "mysticism": 0,
          "necromancy": 0,
          "restoration": 0
        },
        "damage": {
          "fire": 0,
          "frost": 0,
          "shock": 0
        }
      },
      "damage": {
        "fromSunlight": 0,
        "fromSilver": 0,
        "fromMagic": 0
      },
      "tests": {
        "all": 0,
        "fear": 0,
        "social": 0,
        "observe": 0,
        "panic": 0,
        "horror": 0
      },
      "degrees": {
        "success": {
          "all": 0,
          "skills": {
            "all": 0
          },
          "combat": {
            "all": 0,
            "attack": 0,
            "defense": 0
          },
          "magic": {
            "all": 0,
            "alteration": 0,
            "conjuration": 0,
            "destruction": 0,
            "illusion": 0,
            "mysticism": 0,
            "necromancy": 0,
            "restoration": 0
          },
          "social": 0,
          "observe": 0,
          "minimum": {
            "all": 0,
            "skills": {
              "all": 0
            },
            "combat": {
              "attack": 0,
              "defense": 0
            },
            "magic": {
              "all": 0,
              "alteration": 0,
              "conjuration": 0,
              "destruction": 0,
              "illusion": 0,
              "mysticism": 0,
              "necromancy": 0,
              "restoration": 0
            },
            "social": 0,
            "observe": 0
          }
        },
        "failure": {
          "skills": {
            "all": 0
          },
          "combat": {
            "all": 0,
            "attack": 0,
            "defense": 0
          },
          "magic": {
            "all": 0,
            "alteration": 0,
            "conjuration": 0,
            "destruction": 0,
            "illusion": 0,
            "mysticism": 0,
            "necromancy": 0,
            "restoration": 0
          },
          "social": 0,
          "observe": 0,
          "backfire": 0
        }
      }
    }
  },
  "NPC": {
    "templates": [
      "equippedWeapons",
      "favorites",
      "combatTracking"
    ],
    "race": "",
    "birthsign": "",
    "age": "",
    "height": "",
    "weight": "",
    "elite_adv": "",
    "wounded": false,
    "woundPenalty": 0,
    "woundNotes": "",
    "bio": "",
    "paperDoll": "",
    "soul_energy": "",
    "linguistics": {
      "max": 0,
      "known": ""
    },
    "social": {
      "languages": {
        "entries": []
      },
      "factions": []
    },
    "journal": "",
    "notes": "",
    "threat": "",
    "threatCategory": {
      "minorSolo": "Minor Solo",
      "minorGroup": "Minor Group",
      "majorSolo": "Major Solo",
      "majorGroup": "Major Group",
      "deadlySolo": "Deadly Solo",
      "deadlyGroup": "Deadly Group",
      "legendarySolo": "Legendary Solo",
      "legendaryGroup": "Legendary Group"
    },
    "resistance": {
      "physicalR": 0,
      "fireR": 0,
      "frostR": 0,
      "shockR": 0,
      "magicR": 0,
      "poisonR": 0,
      "diseaseR": 0,
      "natToughness": 0,
      "silverR": 0,
      "sunlightR": 0
    },
    "resistances": {
      "disease": 0,
      "poison": 0,
      "paralysis": 0,
      "bleed": 0,
      "fear": 0,
      "magic": 0
    },
    "traits": "",
    "recovery": {
      "naturalHealing": {
        "multiplier": 1,
        "flatBonus": 0
      },
      "magickaRecovery": {
        "multiplier": 1
      },
      "staminaRecovery": {
        "multiplier": 1
      }
    },
    "resources": {
      "magicka": {
        "maxBonus": 0
      },
      "stamina": {
        "maxBonus": 0
      },
      "health": {
        "maxBonus": 0
      }
    },
    "movement": {
      "speedBonus": 0,
      "fallDamage": {
        "multiplier": 1
      },
      "ignoreDifficultTerrain": false
    },
    "senses": {
      "lowLightVision": false,
      "darkvision": false,
      "nightSightBonus": 0
    },
    "fatigue": {
      "level": 0,
      "penalty": 0,
      "bonus": 0
    },
    "lucky_numbers": {
      "ln1": 0,
      "ln2": 0,
      "ln3": 0,
      "ln4": 0,
      "ln5": 0,
      "ln6": 0,
      "ln7": 0,
      "ln8": 0,
      "ln9": 0,
      "ln10": 0
    },
    "unlucky_numbers": {
      "ul1": 0,
      "ul2": 0,
      "ul3": 0,
      "ul4": 0,
      "ul5": 0,
      "ul6": 0
    },
    "supply": "",
    "wealth": 0,
    "current_enc": 0,
    "carry_rating": {
      "current": 0,
      "max": 0,
      "bonus": 0,
      "penalty": 0,
      "label": "Minimal"
    },
    "characteristics": {
      "str": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "end": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "agi": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "int": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "wp": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "prc": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "prs": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      },
      "lck": {
        "value": 0,
        "total": 0,
        "base": 0,
        "favored": false
      }
    },
    "professions": {
      "combat": 0,
      "magic": 0,
      "evade": 0,
      "observe": 0,
      "stealth": 0,
      "knowledge": 0,
      "social": 0,
      "physical": 0,
      "commerce": 0,
      "profession1": 0,
      "profession2": 0,
      "profession3": 0
    },
    "professionsWound": {
      "combat": 0,
      "magic": 0,
      "evade": 0,
      "observe": 0,
      "stealth": 0,
      "knowledge": 0,
      "social": 0,
      "physical": 0,
      "commerce": 0,
      "profession1": 0,
      "profession2": 0,
      "profession3": 0
    },
    "hp": {
      "base": 0,
      "value": 0,
      "max": 0,
      "temp": 0,
      "bonus": 0
    },
    "tempHP": 0,
    "buffers": {
      "physical": 0,
      "magical": 0,
      "elemental": 0
    },
    "action_points": {
      "value": 0,
      "max": 3
    },
    "luck_points": {
      "value": 0,
      "max": 0
    },
    "stamina": {
      "value": 0,
      "max": 0,
      "temp": 0,
      "bonus": 0
    },
    "magicka": {
      "value": 0,
      "max": 0,
      "bonus": 0
    },
    "worship": {
      "primaryDomainKey": "",
      "domains": {}
    },
    "xp": 0,
    "wound_threshold": {
      "value": 0,
      "base": 0,
      "bonus": 0
    },
    "initiative": {
      "value": 0,
      "base": 0,
      "bonus": 0
    },
    "speed": {
      "value": 0,
      "base": 0,
      "bonus": 0
    },
    "size": "",
    "skills": {
      "profession1": {
        "rank": "",
        "bonus": 0,
        "specialization": "",
        "characteristic": "",
        "tn": 0
      },
      "profession2": {
        "rank": "",
        "bonus": 0,
        "specialization": "",
        "characteristic": "",
        "tn": 0
      },
      "profession3": {
        "rank": "",
        "bonus": 0,
        "specialization": "",
        "characteristic": "",
        "tn": 0
      },
      "commerce": {
        "bonus": 0,
        "specialization": "",
        "characteristic": "",
        "tn": 0
      }
    },
    "talents": "",
    "powers": "",
    "spells": "",
    "armor_class": "",
    "modifiers": {
      "combat": {
        "attackTN": 0,
        "evadeAoOCost": 0,
        "attackLimit": {
          "total": 0,
          "melee": 0,
          "ranged": 0
        },
        "opposed": {
          "attackTN": 0
        },
        "defenseTN": {
          "total": 0,
          "evade": 0,
          "block": 0,
          "parry": 0,
          "counter": 0
        }
      },
      "movement": {
        "climbSpeed": 0,
        "dashMultiplier": 0,
        "sprintMultiplier": 0,
        "hiddenSpeedMultiplier": 0
      },
      "magic": {
        "negateChance": 0,
        "spellRestraintBonus": 0,
        "cost": {
          "_all": 0,
          "alteration": 0,
          "conjuration": 0,
          "destruction": 0,
          "illusion": 0,
          "mysticism": 0,
          "necromancy": 0,
          "restoration": 0
        },
        "damage": {
          "fire": 0,
          "frost": 0,
          "shock": 0
        }
      },
      "damage": {
        "fromSunlight": 0,
        "fromSilver": 0,
        "fromMagic": 0
      },
      "tests": {
        "all": 0,
        "fear": 0,
        "social": 0,
        "observe": 0,
        "panic": 0,
        "horror": 0
      },
      "degrees": {
        "success": {
          "all": 0,
          "skills": {
            "all": 0
          },
          "combat": {
            "all": 0,
            "attack": 0,
            "defense": 0
          },
          "magic": {
            "all": 0,
            "alteration": 0,
            "conjuration": 0,
            "destruction": 0,
            "illusion": 0,
            "mysticism": 0,
            "necromancy": 0,
            "restoration": 0
          },
          "social": 0,
          "observe": 0,
          "minimum": {
            "all": 0,
            "skills": {
              "all": 0
            },
            "combat": {
              "attack": 0,
              "defense": 0
            },
            "magic": {
              "all": 0,
              "alteration": 0,
              "conjuration": 0,
              "destruction": 0,
              "illusion": 0,
              "mysticism": 0,
              "necromancy": 0,
              "restoration": 0
            },
            "social": 0,
            "observe": 0
          }
        },
        "failure": {
          "skills": {
            "all": 0
          },
          "combat": {
            "all": 0,
            "attack": 0,
            "defense": 0
          },
          "magic": {
            "all": 0,
            "alteration": 0,
            "conjuration": 0,
            "destruction": 0,
            "illusion": 0,
            "mysticism": 0,
            "necromancy": 0,
            "restoration": 0
          },
          "social": 0,
          "observe": 0,
          "backfire": 0
        }
      }
    }
  },
  "Group": {
    "templates": [
      "equippedWeapons",
      "favorites",
      "combatTracking"
    ],
    "name": "",
    "description": "",
    "notes": "",
    "members": [],
    "travelPace": "normal",
    "travel": {
      "pace": "normal",
      "averageSpeed": 0
    },
    "lastRest": {
      "short": 0,
      "long": 0
    }
  },
  "Warfare Unit": {
    "description": "",
    "notes": "",
    "profile": {
      "id": "uesrpg-0_2"
    },
    "commander": {
      "uuid": "",
      "id": "",
      "name": "",
      "img": "",
      "bonusOverride": 0
    },
    "commanderAttachment": {
      "leaderActorUuid": "",
      "warfareTokenUuid": "",
      "leaderTokenUuid": "",
      "sceneId": ""
    },
    "identity": {
      "category": "",
      "ancestry": "",
      "rank": ""
    },
    "doctrine": {
      "tradition": ""
    },
    "composition": {
      "racialPresetKey": "",
      "racialMods": {
        "speedMod": 0,
        "magickaMod": 0,
        "offenseMod": 0,
        "offenseType": "",
        "conditionMod": 0,
        "disciplineMod": 0,
        "special": ""
      }
    },
    "mounts": {
      "primary": "none"
    },
    "economy": {
      "cadence": "weekly",
      "mode": "gold",
      "amount": 0,
      "enslaved": false,
      "unpaidWeeks": 0,
      "specialModifier": 0
    },
    "magic": {
      "mode": "implements",
      "entries": []
    },
    "equipment": {
      "owned": []
    },
    "variant": {
      "tags": [],
      "overrides": {}
    },
    "status": {
      "leaderless": false,
      "battle": {
        "hidden": false,
        "ambushReady": false,
        "revealed": true,
        "routed": false,
        "broken": false,
        "suppressed": false,
        "defeated": false,
        "frenzied": false,
        "flyer": false
      }
    },
    "modifiers": {
      "discipline": {
        "manual": 0,
        "campaign": {
          "inspiringSpeech": false,
          "forcedMarch": false,
          "poorClimate": false,
          "longCampaign": false,
          "defendingAlliedSettlement": false
        },
        "battle": {
          "rearCharged": false,
          "adjacentFriendlyBroken": false,
          "commanderLost": false,
          "rallyBonus": false,
          "enemyBrokenBonus": false
        }
      }
    },
    "classification": {
      "unitType": "",
      "ancestry": "",
      "mount": "none",
      "tier": "light"
    },
    "stats": {
      "bulk": {
        "value": 1,
        "max": 0,
        "lossTotal": 0
      },
      "resolve": {
        "value": 0,
        "max": 0,
        "lossTotal": 0
      },
      "discipline": {
        "value": 0,
        "base": 0,
        "bonus": 0
      },
      "condition": {
        "value": 0,
        "max": 0,
        "maxOverride": 0,
        "useMaxOverride": false
      },
      "magicka": {
        "value": 0,
        "max": 0
      },
      "speed": {
        "value": 0,
        "base": 0,
        "bonus": 0
      }
    },
    "gear": {
      "tier": "light",
      "apparel": "light",
      "sets": 0,
      "dmg": "",
      "ar": 0,
      "mar": 0,
      "speedPenalty": 0,
      "cost": 0
    },
    "racial": {
      "speedMod": 0,
      "magickaMod": 0,
      "offenseMod": 0,
      "offenseType": "",
      "conditionMod": 0,
      "disciplineMod": 0,
      "special": ""
    },
    "combat": {
      "hidden": false,
      "deployed": false,
      "leaderless": false
    },
    "traits": [],
    "deployableEquipment": [],
    "spells": [],
    "upkeep": {
      "weeklyGold": 0,
      "enslaved": false
    },
    "rules": {
      "source": "UESRPG Mass Warfare 3e",
      "version": "2.0"
    }
  }
};

export const ITEM_TYPE_MODEL_SEEDS = {
  "equipment": {
    "templates": [
      "characteristicArray",
      "replaceAttributesArray",
      "resistanceArray",
      "objectArrays",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "reach": 0,
    "damage": "",
    "equipped": false,
    "hands": 0,
    "price": 0,
    "armor": 0,
    "magic_ar": 0,
    "blockRating": 0,
    "enchant_level": 0,
    "consumable": false,
    "qualities": "",
    "wearable": false,
    "charge": {
      "value": 0,
      "max": 0,
      "reduction": 1
    }
  },
  "item": {
    "templates": [
      "characteristicArray",
      "replaceAttributesArray",
      "resistanceArray",
      "objectArrays",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "reach": 0,
    "damage": "",
    "equipped": false,
    "hands": 0,
    "price": 0,
    "armor": 0,
    "magic_ar": 0,
    "blockRating": 0,
    "enchant_level": 0,
    "consumable": false,
    "qualities": "",
    "wearable": false,
    "charge": {
      "value": 0,
      "max": 0,
      "reduction": 1
    }
  },
  "container": {
    "templates": [
      "characteristicArray",
      "replaceAttributesArray",
      "resistanceArray",
      "objectArrays",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "container_enc": {
      "current": 0,
      "max": 0,
      "applied_enc": 0,
      "item_count": 0
    },
    "enc_reduction": 0,
    "equipped": false,
    "price": 0,
    "enchant_level": 0,
    "qualities": "",
    "wearable": true,
    "contained_items": []
  },
  "armor": {
    "templates": [
      "resistanceArray",
      "characteristicArray",
      "replaceAttributesArray",
      "objectArrays",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "reach": 0,
    "damage": "",
    "equipped": false,
    "carried": false,
    "qualityLevel": "common",
    "runed": false,
    "runedBonusApplied": false,
    "armorClass": "partial",
    "isShield": false,
    "shieldType": "normal",
    "special_ar_type": "",
    "special_ar": 0,
    "magic_br": 0,
    "hands": 0,
    "price": 0,
    "armor": 0,
    "blockRating": 0,
    "magic_ar": 0,
    "enchant_level": 0,
    "qualities": "",
    "category": "",
    "item_cat": {
      "none": "None",
      "head": "Helmet",
      "l_arm": "Left Gauntlet",
      "r_arm": "Right Gauntlet",
      "l_leg": "Left Leg",
      "r_leg": "Right Leg",
      "body": "Chestplate",
      "shield": "Shield"
    },
    "hitLocations": {
      "Head": false,
      "Body": false,
      "RightArm": false,
      "LeftArm": false,
      "RightLeg": false,
      "LeftLeg": false
    },
    "charge": {
      "value": 0,
      "max": 0,
      "reduction": 1
    },
    "weightClass": "none",
    "material": "standard",
    "qualitiesStructured": [],
    "qualitiesTraits": [],
    "effectiveWeightClass": "none",
    "armorValues": {
      "full": {
        "armor": 0,
        "magic_ar": 0,
        "special_ar": 0,
        "special_ar_type": ""
      },
      "partial": {
        "armor": 0,
        "magic_ar": 0,
        "special_ar": 0,
        "special_ar_type": ""
      }
    }
  },
  "shield": {
    "templates": [
      "resistanceArray",
      "characteristicArray",
      "replaceAttributesArray",
      "objectArrays",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "equipped": false,
    "carried": false,
    "qualityLevel": "common",
    "runed": false,
    "runedBonusApplied": false,
    "shieldType": "normal",
    "magic_br": 0,
    "hands": 0,
    "price": 0,
    "blockRating": 0,
    "enchant_level": 0,
    "qualities": "",
    "charge": {
      "value": 0,
      "max": 0,
      "reduction": 1
    },
    "weightClass": "none",
    "material": "standard",
    "qualitiesStructured": [],
    "qualitiesTraits": [],
    "effectiveWeightClass": "none",
    "blockRatingEffective": 0,
    "magic_brEffective": 0,
    "magic_brSpecial": null,
    "enchant_levelEffective": 0,
    "treatAsFreeHandForSmallOrGrapple": false
  },
  "weapon": {
    "templates": [
      "resistanceArray",
      "characteristicArray",
      "replaceAttributesArray",
      "objectArrays",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "reach": 0,
    "reachMin": 0,
    "range": "",
    "damage": "",
    "damage2": "",
    "damage3": "",
    "weapon2H": false,
    "equipped": false,
    "carried": false,
    "superior": false,
    "attackMode": "melee",
    "ammoId": "",
    "consumeAmmo": true,
    "hands": 0,
    "price": 0,
    "armor": 0,
    "magic_ar": 0,
    "enchant_level": 0,
    "qualities": "",
    "charge": {
      "value": 0,
      "max": 0,
      "reduction": 1
    },
    "qualityLevel": "common",
    "material": "standard",
    "qualitiesStructured": [],
    "qualitiesTraits": [],
    "equipmentType": "",
    "reloadState": {
      "isLoaded": true,
      "reloadAPCost": 0,
      "requiresReload": false,
      "reloadProgress": 0
    }
  },
  "spell": {
    "description": "",
    "damage": "",
    "damageType": "none",
    "damageTypes": {
      "none": "None",
      "fire": "Fire",
      "frost": "Frost",
      "shock": "Shock",
      "poison": "Poison",
      "magic": "Magic",
      "physical": "Physical",
      "healing": "Healing",
      "temporaryHealing": "Temporary Healing"
    },
    "damageFormula": "",
    "damageInstances": [],
    "healAmount": "",
    "level": 0,
    "cost": 0,
    "spell_str": "",
    "attributes": "",
    "form": "",
    "school": "",
    "spellType": "conventional",
    "spellTypes": {
      "conventional": "Conventional",
      "unconventional": "Unconventional"
    },
    "isAttackSpell": false,
    "isDamagingSpell": false,
    "isHealingSpell": false,
    "isInstant": false,
    "hasUpkeep": false,
    "hasOverload": false,
    "overloadEffect": "",
    "overloadBonusDamage": "",
    "hasReinforce": false,
    "reinforceDescription": "",
    "rangeType": "",
    "range": "",
    "aoeShape": "circle",
    "aoeSize": 0,
    "aoeWidth": 0,
    "aoePulse": false,
    "aoeIncludeCaster": false,
    "isDirect": false,
    "mindlockValue": 0,
    "isRuneSpell": false,
    "runeTriggerType": "",
    "runeTriggerRadius": 3,
    "runeTriggerDelay": 0,
    "isSummonSpell": false,
    "isZonePersistent": false,
    "hasOverTime": false,
    "overTime": {
      "trigger": "turnStart",
      "cadenceEvery": 1,
      "cadenceUnit": "rounds",
      "payloadType": "damage",
      "formula": "1d6",
      "damageType": "fire",
      "ignoreReduction": false,
      "saveKey": "",
      "saveTN": 0,
      "saveSuccess": "endEffect",
      "saveFailure": "damage",
      "maxTicks": null,
      "label": "",
      "chatLog": true
    },
    "overTimeEntries": [],
    "hasBuffer": false,
    "buffer": {
      "type": "none",
      "types": {
        "none": "None",
        "physical": "Physical",
        "magical": "Magical",
        "elemental": "Elemental"
      },
      "formula": "",
      "freeUpkeepIfRemains": false
    },
    "isSpellDefense": false,
    "spellDefenseType": "none",
    "spellDefenseTypes": {
      "none": "None",
      "absorption": "Spell Absorption",
      "reflect": "Spell Reflect"
    },
    "duration": {
      "value": 0,
      "unit": "rounds",
      "units": {
        "instant": "Instant",
        "rounds": "Rounds",
        "minutes": "Minutes",
        "hours": "Hours",
        "days": "Days",
        "permanent": "Permanent"
      }
    },
    "scaling": {
      "levels": []
    },
    "engine": {
      "targeting": {
        "mode": "single",
        "maxTargets": 1
      },
      "effects": {
        "recipes": [],
        "stackingPolicy": "replace",
        "ownershipPolicy": "target"
      },
      "persistence": {
        "dispelStrength": "level",
        "dispelFixedValue": 0
      },
      "summon": {
        "actorUuid": "",
        "quantity": 1
      },
      "conjure": {
        "mode": "none",
        "itemUuid": "",
        "itemLabel": "",
        "actorUuid": "",
        "actorLabel": "",
        "bindingCharacteristic": "wp",
        "bindingModifier": 0,
        "summonItems": null,
        "summonActors": null
      },
      "disintegrate": {
        "enabled": false,
        "target": "armor"
      },
      "drain": {
        "enabled": false,
        "type": "none",
        "transferToCaster": false
      },
      "defenseModel": "opposed",
      "characteristicDefense": {
        "defenderCharacteristic": "end",
        "modifierMode": "spellStrength",
        "modifierFormula": "",
        "onSuccess": "negate",
        "onFailure": "consequences"
      },
      "consequences": {
        "staminaDelta": 0,
        "healthDelta": 0,
        "magickaDelta": 0,
        "applyCondition": "",
        "description": ""
      },
      "cloak": {
        "enabled": false,
        "range": 1,
        "excludeSelf": true,
        "requireAttackTest": false,
        "useSpellDamage": true
      }
    }
  },
  "trait": {
    "templates": [
      "resistanceArray",
      "replaceAttributesArray",
      "characteristicArray",
      "objectArrays"
    ],
    "description": "",
    "halfSpeed": false,
    "addHalfSpeed": false,
    "halfFatiguePenalty": false,
    "mechanical": false,
    "crpCost": 0,
    "traitKey": "",
    "traitParam": "",
    "traitValue": 0,
    "traitUnit": "",
    "traitNotes": "",
    "halfWoundPenalty": false,
    "painIntolerant": false,
    "activation": {
      "enabled": false,
      "actionType": "passive",
      "spendCosts": false,
      "consumeUse": false,
      "costs": {
        "action_points": 0,
        "stamina": 0,
        "magicka": 0,
        "luck_points": 0,
        "health": 0
      },
      "requirements": {
        "requiresTarget": false,
        "requiresEquippedWeapon": false,
        "requiresMelee": false,
        "requiresRanged": false,
        "requiresHitLocation": false
      },
      "flags": {
        "npcLuckAllowed": false,
        "npcEliteAllowed": false
      },
      "usage": {
        "max": 0,
        "period": "",
        "current": 0
      },
      "roll": {
        "mode": "none",
        "hitLocationMode": "roll",
        "skillKey": "",
        "opposed": {
          "actor": "target",
          "skillKey": ""
        },
        "formula": "",
        "tags": []
      },
      "damage": {
        "mode": "weapon",
        "formula": "",
        "type": "physical",
        "qualitiesStructured": [],
        "qualitiesTraits": []
      },
      "text": {
        "short": "",
        "full": ""
      }
    }
  },
  "power": {
    "templates": [
      "resistanceArray",
      "characteristicArray",
      "objectArrays"
    ],
    "description": "",
    "dailyUse": false,
    "source": "",
    "shiftForm": false,
    "shiftFormStyle": "",
    "shiftStyles": {
      "shiftFormNone": "Regular Form",
      "shiftFormVampireLord": "Vampire Lord",
      "shiftFormWereWolf": "Werewolf",
      "shiftFormWereBat": "Werebat",
      "shiftFormWereBoar": "Wereboar",
      "shiftFormWereBear": "Werebear",
      "shiftFormWereCrocodile": "Werecrocodile",
      "shiftFormWereVulture": "Werevulture",
      "shiftFormWereLion": "Werelion"
    },
    "rangeType": "",
    "range": "",
    "aoeShape": "circle",
    "aoeSize": 0,
    "aoeWidth": 0,
    "aoePulse": false,
    "aoeIncludeCaster": false,
    "activation": {
      "enabled": false,
      "actionType": "action",
      "spendCosts": false,
      "consumeUse": false,
      "costs": {
        "action_points": 0,
        "stamina": 0,
        "magicka": 0,
        "luck_points": 0,
        "health": 0
      },
      "uses": {
        "value": 0,
        "max": 0,
        "reset": "none"
      },
      "requirements": {
        "requiresTarget": false,
        "requiresEquippedWeapon": false,
        "requiresMelee": false,
        "requiresRanged": false,
        "requiresHitLocation": false
      },
      "usage": {
        "max": 0,
        "period": "",
        "current": 0
      },
      "roll": {
        "mode": "none",
        "hitLocationMode": "roll",
        "skillKey": "",
        "opposed": {
          "actor": "target",
          "skillKey": ""
        },
        "formula": "",
        "tags": []
      },
      "damage": {
        "mode": "weapon",
        "formula": "",
        "type": "physical",
        "qualitiesStructured": [],
        "qualitiesTraits": []
      },
      "text": {
        "short": "",
        "full": ""
      }
    }
  },
  "talent": {
    "templates": [
      "resistanceArray",
      "replaceAttributesArray",
      "characteristicArray",
      "objectArrays"
    ],
    "description": "",
    "governingCharacteristics": "",
    "xpCost": 0,
    "level": "",
    "talentReq": "",
    "miscReq": "",
    "halfFatiguePenalty": false,
    "halfWoundPenalty": false,
    "untrainedException": false,
    "fatigueException": false,
    "addIBToMP": false,
    "addIntToMPMultiplier": 1,
    "armorAcrobaticsException": false,
    "addHalfSpeed": false,
    "doubleSwimSpeed": false,
    "activation": {
      "enabled": false,
      "actionType": "passive",
      "spendCosts": false,
      "consumeUse": false,
      "costs": {
        "action_points": 0,
        "stamina": 0,
        "magicka": 0,
        "luck_points": 0
      },
      "requirements": {
        "requiresTarget": false,
        "requiresEquippedWeapon": false,
        "requiresMelee": false,
        "requiresRanged": false,
        "requiresHitLocation": false
      },
      "usage": {
        "max": 0,
        "period": "",
        "current": 0
      },
      "roll": {
        "mode": "none",
        "hitLocationMode": "roll",
        "skillKey": "",
        "opposed": {
          "actor": "target",
          "skillKey": ""
        },
        "formula": "",
        "tags": []
      },
      "damage": {
        "mode": "weapon",
        "formula": "",
        "type": "physical",
        "qualitiesStructured": [],
        "qualitiesTraits": []
      },
      "text": {
        "short": "",
        "full": ""
      }
    }
  },
  "combatStyle": {
    "templates": [
      "characteristicArray"
    ],
    "value": 0,
    "rank": "",
    "bonus": 0,
    "miscValue": 0,
    "specialization": "",
    "description": "",
    "baseCha": "",
    "trainedItems": "",
    "governingCha": "",
    "trainedEquipment": [
      "",
      "",
      "",
      "",
      "",
      "",
      "",
      "",
      "",
      ""
    ],
    "specialAdvantages": {}
  },
  "skill": {
    "templates": [
      "characteristicArray"
    ],
    "value": 0,
    "rank": "",
    "bonus": 0,
    "miscValue": 0,
    "specialization": "",
    "description": "",
    "baseCha": "",
    "governingCha": "",
    "trainedItems": ""
  },
  "magicSkill": {
    "templates": [
      "characteristicArray"
    ],
    "value": 0,
    "rank": "",
    "bonus": 0,
    "miscValue": 0,
    "specialization": "",
    "description": "",
    "baseCha": "",
    "governingCha": "",
    "trainedItems": ""
  },
  "ammunition": {
    "templates": [
      "resistanceArray",
      "characteristicArray",
      "replaceAttributesArray",
      "physicalObject"
    ],
    "description": "",
    "quantity": 1,
    "enc": 0,
    "totalENC": 0,
    "damage": 0,
    "carried": false,
    "equipped": false,
    "superior": false,
    "price": 0,
    "armor": 0,
    "magic_ar": 0,
    "enchant_level": 0,
    "qualities": "",
    "charge": {
      "value": 0,
      "max": 0
    },
    "arrowType": "none",
    "damageType": "",
    "qualitiesStructured": [],
    "qualitiesTraits": []
  },
  "invocation": {
    "description": "",
    "domainKey": "",
    "tnDomainKey": "",
    "circle": 1,
    "pietyCost": 1,
    "isUniversal": false,
    "time": "",
    "ritual": "",
    "range": "",
    "duration": "",
    "aspects": [],
    "requirements": "",
    "effect": "",
    "automation": {
      "mode": "manual",
      "key": "",
      "descriptors": [],
      "allowManualFallback": true
    },
    "source": {
      "sourceKey": "",
      "sourcePath": "",
      "importVersion": 1
    }
  },
  "scroll": {
    "description": "",
    "quantity": 1,
    "enc": 0,
    "spellUuid": "",
    "requireSchoolTraining": false,
    "consumeMagicka": false,
    "consumeOnCast": true,
    "spellData": {}
  }
};

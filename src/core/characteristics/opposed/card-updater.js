/**
 * src/core/characteristics/opposed/card-updater.js
 *
 * Centralized card-update function for characteristic opposed workflows.
 * Mirrors the combat/magic/skills updater pattern with:
 ...
 * @param {Object} data - The characteristic opposed workflow data object.
 * @returns {Promise<void>}
 */
...
async function _updateCardCore(message, data) {
  ...
  const renderLabel = `card.update.render:char:${msgId}`;
  ...
  const persistLabel = `card.update.persist:char:${msgId}`;

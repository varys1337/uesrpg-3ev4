...
const _STATUS_HUD_UPGRADE_MAX_STARTUP_RETRIES = 6;
...
export function registerSystemStatusEffects() {
  ...
  const existing = getStatusEffectConfigs();
  ...
  setStatusEffectConfigs(Array.from(byId.values()).filter(e => e && typeof e.id === "string" && !excluded.has(String(e.id).toLowerCase())));
}
...
function _statusHudDebugEnabled() {
  return game.user?.isGM && isDebugEnabled("effectsProxyDebug");
}

function _statusHudDebug(message, details = null) {
  if (!_statusHudDebugEnabled()) return;
  try {
    if (details == null) console.log("[UESRPG][StatusHUD]", message);
    else console.log("[UESRPG][StatusHUD]", message, details);
  } catch (_e) {
    // Non-blocking.
  }
}

function _requestAnimationFrameSafe() {
...
function _isCombatTurnMarkerMeshPending() {
  const token = _getViewedCombatantToken();
  if (!token?.turnMarker) return false;
  return !token.turnMarker.mesh;
}

async function _waitForCombatSafeStartupWindow({ maxRetries = _STATUS_HUD_UPGRADE_MAX_STARTUP_RETRIES } = {}) {
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    if (!canvas?.ready) {
      await _requestAnimationFrameSafe();
      continue;
    }

    await _requestAnimationFrameSafe();

    if (!_isCombatTurnMarkerMeshPending()) return true;

    _statusHudDebug("Token HUD upgrade sweep deferred because combat turn marker mesh was not ready", {
      combatId: game.combat?.id ?? null,
      tokenId: _getViewedCombatantToken()?.id ?? null,
      attempt: attempt + 1,
  ...
  _statusHudDebug("Token HUD upgrade sweep aborted before completion because combat turn marker mesh stayed unavailable", {
    combatId: game.combat?.id ?? null,
    tokenId: _getViewedCombatantToken()?.id ?? null,
...
export async function runTokenHudStatusUpgradeMaintenance() {
  try {
    ...
    const actorsNeedingUpgrade = Array.from(game.actors ?? []).filter(actor => _actorNeedsTokenHudStatusUpgrade(actor));
    if (actorsNeedingUpgrade.length < 1) {
      await game.settings.set(systemId, "conditionsHudUpgradeV2Done", true);
      _statusHudDebug("Completed actor upgrade sweep", { actorCount: 0 });
      return;
    }

    const startupWindowReady = await _waitForCombatSafeStartupWindow();
    if (!startupWindowReady) return;

    for (const actor of actorsNeedingUpgrade) {
      await upgradeTokenHudStatusEffects(actor);
    }

    await game.settings.set(systemId, "conditionsHudUpgradeV2Done", true);
    _statusHudDebug("Completed actor upgrade sweep", { actorCount: actorsNeedingUpgrade.length });
  } catch (err) {
    _statusHudDebug("Aborted actor upgrade sweep before completion", {
